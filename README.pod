...
I'd be happy to answer any questions about this project or L<via email|mailto:jmac@jmac.org>. You may also wish to join the L<"#whim" channel on Freenode IRC|http://webchat.freenode.net/?channels=%23whim>, where I am likely idling but listening as C<jmac>.
...
Besides listening for incoming webmentions, the C<listen> command also sets up HTTP endponts at C</display_wms> and C</summarize_wms>. They accept GET requests that contain one query-string argument, C<url>. Whim will fetch and display, as HTML, all verified webmentions whose source matches the given URL.
